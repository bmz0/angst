...
// lib.dom.d.ts (included via tsconfig.worklet.json) supplies AudioWorkletNodeOptions,
// MessagePort, and MessageEvent.  The types below are specific to the
// AudioWorkletGlobalScope and are absent from lib.dom.d.ts; they must be
// declared manually.  All declarations are erased at compile time.

/** Descriptor for an AudioWorkletProcessor's AudioParam. */
...
/** Base class available as a global in the AudioWorkletGlobalScope. */
...
/** Registers a processor in the AudioWorkletGlobalScope. */
declare function registerProcessor(
  name: string,
  processorCtor: new (options?: AudioWorkletNodeOptions) => AudioWorkletProcessor
): void;

/** Sample rate of the AudioContext, available as a global in the worklet scope. */
 ...
 */
function computeG(cutoffHz: number): number {
  return Math.min(
    0.9999,
    Math.max(1e-4, Math.tan((Math.PI * cutoffHz) / sampleRate))
  );
}
 ...
 */
function tanhClip(x: number): number {
  if (x > 3)  return  1;
  if (x < -3) return -1;
  const x2 = x * x;
